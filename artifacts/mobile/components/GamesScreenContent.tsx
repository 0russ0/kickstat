...
export function GamesScreenContent({ onClose }: { onClose: () => void }) {
  ...
  const seasonsQuery = useGetSeasons(
    ...
    { query: { queryKey: getGetSeasonsQueryKey({ athleteId: activeAthleteId ?? "" }), enabled: !!activeAthleteId } },
  );
  const gamesQuery = useGetGames(
    { seasonId: expandedSeason ?? "" },
    { query: { queryKey: getGetGamesQueryKey({ seasonId: expandedSeason ?? "" }), enabled: !!expandedSeason } },
  );
  ...
  const handleCreateSeason = async () => {
    ...
    invalidateSeasons();
    setSeasonName(""); setSeasonYear(String(new Date().getFullYear())); setShowSeasonModal(false);
  };

  const openEditSeason = (season: Season) => {
    setEditingSeason(season); setEditSeasonName(season.name); setEditSeasonYear(String(season.year)); setShowEditSeasonModal(true);
  };
  ...
  const handleEditSeasonSave = async () => {
    ...
    try {
      ...
      invalidateSeasons(); setShowEditSeasonModal(false); setEditingSeason(null);
    } finally { setSavingEditSeason(false); }
  };
  ...
  const handleCreateGame = async () => {
    ...
    await createGameMutation.mutateAsync({ data: {
      seasonId: gameSeasonId, athleteId: activeAthleteId,
      opponent: gameOpponent.trim() || null, date: gameDate,
      homeAway: gameHomeAway, surface: gameSurface,
      weather: { conditions: gameWeatherCond, windMph: gameWindMph ? parseInt(gameWindMph, 10) : null, windDir: null },
      isPlayoff: gameIsPlayoff,
    }});
    invalidateGames(); setShowGameModal(false); resetGameForm();
  };

  const resetGameForm = () => {
    setGameOpponent(""); setGameDate(new Date().toISOString().slice(0, 10));
    setGameHomeAway("home"); setGameSurface("grass"); setGameWeatherCond("clear");
    setGameWindMph(""); setGameIsPlayoff(false);
  };
  ...
  const handleDeleteGame = (game: Game) => {
    Alert.alert("Delete Game", "Delete this game and all its kicks?", [
      { text: "Cancel", style: "cancel" },
      { text: "Delete", style: "destructive", onPress: async () => {
        await deleteGameMutation.mutateAsync({ id: game.id }); invalidateGames();
      }},
    ]);
  };
  ...
  const handleSaveScore = async () => {
    ...
    await updateGameMutation.mutateAsync({ id: scoringGame.id, data: {
      myScore: myScore ? parseInt(myScore, 10) : null,
      opponentScore: oppScore ? parseInt(oppScore, 10) : null,
    }});
    invalidateGames(); setShowScoreModal(false); setScoringGame(null);
  };
                    ...
                    games.map((game, i) => (
                      <View key={game.id}>
                        ...
                        <View style={s.gameRow}>
                          ...
                          <Text style={s.gameMeta}>{formatDate(game.date)} · {game.surface} · {game.homeAway}</Text>
                          {game.weather && (
                            <Text style={s.gameMeta}>
                              {game.weather.conditions ?? ""}{game.weather.windMph ? `  · ${game.weather.windMph} mph wind` : ""}
                            </Text>
                          )}
